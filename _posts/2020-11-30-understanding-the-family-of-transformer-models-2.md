...
The quality of summaries is evaluated by ROUGE scores. Using only the in-domain pre-training stage, the $$\mathrm{HIBERT}_{S}$$ (in-domain) significantly outperforms all previous models, including $$BERT_{BASE}$$ that contains double number of model parameters ($$\mathrm{HIBERT}_{S}$$ 54.6M vs $$BERT_{BASE}$$ 110M). With both open-domain and in-domain pre-training stages, $$\mathrm{HIBERT}_{S}$$ and $$\mathrm{HIBERT}_{M}$$ perform even better. Although $$\mathrm{HIBERT}_{M}$$ achieves new state-of-the-art performance, it still lags behind human.
